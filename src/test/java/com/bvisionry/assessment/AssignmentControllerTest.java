...
class AssignmentControllerTest {
    ...
    void createAssignment_returns201() throws Exception {
        ...
        CreateAssignmentRequest request = new CreateAssignmentRequest(
                pipelineId, null, null, null, false, null);

        AssignmentResponse response = new AssignmentResponse(
                UUID.randomUUID(), pipelineId, "Test Pipeline", orgId,
    ...
    void listAssignments_returns200() throws Exception {
        ...
        AssignmentResponse response = new AssignmentResponse(
                UUID.randomUUID(), UUID.randomUUID(), "Pipeline", orgId,
                userId, "Bob", "bob@test.com",
                UUID.randomUUID(), null, UUID.randomUUID(), SubmissionStatus.EVALUATED,
                Instant.now(), 1, 1);

        when(assignmentService.listAssignments(orgId)).thenReturn(List.of(response));
