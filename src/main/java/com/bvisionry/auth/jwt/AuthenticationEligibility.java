 ...
 *
 * <p>It exists because two filters had drifted: {@link JwtAuthenticationFilter}
 * refused a non-{@link UserStatus#ACTIVE} user, a now-deleted second filter (the
 * download-token capability; see git history / agent-decisions.md "HARDENING SWEEP")
 * did not — so the same suspended account was rejected on the cookie path and accepted
 * on the other. Kept as a shared predicate rather than folded back inline: a future
 * filter is free to drift the same way a copy-pasted check would; one call site cannot.
 *
