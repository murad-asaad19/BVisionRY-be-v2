...
## WAVE 10 CLOSED — 51 tickets. One resource, one URL.
`org_url_unification` (be `6d96d43`, web `c2ff626`) collapsed the parallel
`/organizations/[orgId]` and `/sub-organizations/[subOrgId]` admin trees, moved tab/filter state
into the URL (P2-12), and left the "orphan" route alone (P2-13) — see below.

**Combination gates:** backend `./mvnw clean test` **1248 / 0F / 0E**; web lint 0, typecheck 0,
**819 tests / 66 files**; frozen store **0 added, 1 pruned**; **Gate 4 157 passed x2 consecutive**.

**This was a GUARD UNION, not a rename, and that is the whole risk.** The two families rendered the
same components over the same org but did NOT share a guard: family A was `requireSuperAdmin`,
family B was `requireRole([ORG_ADMIN, SUPER_ADMIN])` plus a parent-of-sub-org check. The effective
access set is unchanged for every resource except one, which NARROWED: a viewer with a null
`orgId` used to reach a root org because the old layout evaluated `null !== null` as false. Root
consoles and the `settings`/`sub-organizations` tabs remain SUPER_ADMIN-only. Verified the shared
`requireOrgConsoleAccess` myself: `parentOrganizationId != null` guards BOTH branches, which is
what keeps root orgs super-admin-only, and a null session org now falls through to `notFound()`.
Worker mutation-tested it — 4 of 11 red with the check disabled, including the cross-tenant case.

**A live defect fell out of the merge:** `isSuperAdmin` was hardcoded per tree, so the same super
admin got `true` at `/organizations/{id}/members` and `false` at `/sub-organizations/{id}/members`
— different powers over one org depending which URL they typed. Now read from the session.

**Old bookmarks 307, not 404.** 307 over 308 deliberately: these routes are behind auth so a
permanent redirect buys no SEO, and a 308 is browser-cached ~forever. `/app/admin/sub-organizations`
with NO id is a different resource (session-scoped index, the org admin's sidebar anchor) and is
deliberately NOT redirected — doing so would have required admitting org admins to their own ROOT
org console, the exact widening this ticket bans.

### DOCTRINE — the stale-generated-artifact trap has a second form
Wave 9 was faked red by a stale `target/classes` entry. Wave 10 was faked red by a stale
`.next/dev/types/validator.ts`, generated by the dev server I ran for Gate 4 and still naming the
pages this wave deleted: 12 phantom TS2307s in a file no human wrote, while the lane's own
typecheck was green. **Both gate commands must clear generated output first — `mvnw CLEAN test`
and `rm -rf .next` before typecheck — whenever a wave moves or deletes a file.**

### Worker corrections ratified (eleven and twelve)
1. **The orphan route is REACHABLE; leaving it was correct.** `/app/admin/exercises` matches the
   brief's test (absent from nav, zero references across `src` and `e2e`) but
   `ExerciseSubmissionService` push-notifies BOTH roles to it on submit and on comment reply, so
   the notification bell reaches it. P2-13 is therefore NOT done, on evidence rather than on
   preference.
2. **A backend change WAS needed, against my guess of none.** Two notification types deep-linked
   org admins to `/app/admin/members` and `/app/admin/assignments` — pages that do not exist. Every
   org admin clicking those hit a 404.

### New, unfixed, and the operator's call
`/app/admin/exercises` does `if (!session.orgId) notFound()`, and `Session.orgId` is null for
SUPER_ADMIN — so the **super-admin arm of that very notification 404s today**. It is also the wrong
resource for org admins: it shows their own org's exercises, not the sub-org where the submission
happened. The worker declined to fix it because the fix strips its last inbound reference and
manufactures the orphan — converting "leave it" into "delete it" on its own authority. Correct
call. Two-line change plus a page deletion, when someone decides.

### Constraint list amendment
The `MemberService` frozen-constructor trap generalises: it is not only NEW parameters. **Adding a
